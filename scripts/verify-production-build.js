...
// Check Dockerfile
const dockerfilePath = path.resolve(__dirname, '../infrastructure/docker/unified.Dockerfile');
if (checkFile(dockerfilePath, 'Unified Dockerfile')) {
  ...
  if (dockerfileContent.includes('COPY --from=builder /app/build/frontend /app/apps/frontend/dist')) {
...
if (checkFile(lockfilePath, 'pnpm-lock.yaml')) {
  ...
  if (lockfileSize > 0) {
    log(`  ✓ pnpm-lock.yaml is valid (${Math.round(lockfileSize / 1024)}KB)`, GREEN);
  } else {
...
if (checkFile(renderYamlPath, 'Render configuration')) {
  ...
  if (renderContent.includes('dockerfilePath: ./infrastructure/docker/unified.Dockerfile')) {
