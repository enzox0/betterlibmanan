...
import { listNotes, deleteNoteRequest, type AdminNote } from '../../services/freedom-wall.api';
...
function EmptyState() {
  return (
    <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50/50 py-14 text-center">
      ...
      <p className="text-sm text-gray-400">No notes posted yet on the Freedom Wall.</p>
    </div>
  );
}
...
export function FreedomWallLayout({ onCountChange }: { onCountChange?: (count: number) => void }) {
  const accessToken = useAdminStore((s) => s.accessToken);
  const [notes, setNotes]       = useState<AdminNote[]>([]);
  const [query, setQuery]       = useState('');
  const [loading, setLoading]   = useState(true);
  const [error, setError]       = useState<string | null>(null);
  const [confirm, setConfirm]   = useState<ConfirmState>({ open: false, note: null });
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white py-14 text-center">
        ...
        <p className="text-xs text-gray-400 uppercase tracking-widest">Loading notes…</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-gray-50/80 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500">
            <tr>
              ...
              <th className="px-4 py-2.5 font-semibold w-44 hidden md:table-cell">Posted</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.length === 0 ? (
              ...
              filtered.map((note) => (
              <tr key={note._id} className="hover:bg-gray-50/40 transition-colors">
      ...
      <AnimatePresence>
        {confirm.open && confirm.note && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={(e) => { if (e.target === e.currentTarget && !deleting) setConfirm({ open: false, note: null }); }}
          >
            <motion.div
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <div className="h-10 w-10 rounded-full bg-red-50 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="h-5 w-5 text-red-500" aria-hidden="true">
                ...
                <div>
                  <h3 className="text-base font-bold text-gray-900">Remove this note?</h3>
              ...
              <div className="flex gap-3">
                ...
                >
                  {deleting && <span className="h-3.5 w-3.5 rounded-full border-2 border-white/40 border-t-white animate-spin" />}
