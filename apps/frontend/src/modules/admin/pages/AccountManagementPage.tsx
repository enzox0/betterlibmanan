...
const STATUS_META: Record<AccountStatus, { label: string; dotClass: string; classes: string }> = {
  active: {
    label: 'Active',
...
function AccountFormModal({ mode, initialData, onClose, onSave }: AccountFormModalProps) {
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div>
              <h2 id="account-form-title" className="text-base font-bold text-gray-900">
                {isCreate ? 'New Account' : 'Edit Account'}
              </h2>
              <p className="mt-0.5 text-xs text-gray-400">
                {isCreate ? 'Set up a new admin account.' : 'Update account details.'}
              </p>
            </div>
            <button
              type="button"
          ...
          <form id="account-form" onSubmit={handleSubmit} noValidate>
            <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
              {/* Name */}
              <div>
                <label htmlFor="af-name" className="block text-sm font-medium text-gray-700 mb-1.5">
                ...
                />
                {errors.name && <p id="af-name-error" role="alert" className="mt-1 text-xs text-red-600">{errors.name}</p>}
              </div>

              {/* Username + Email */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="af-username" className="block text-sm font-medium text-gray-700 mb-1.5">
                  ...
                  <input
                    ...
                    aria-describedby={errors.username ? 'af-username-error' : undefined}
                    className={errors.username ? inputError : inputNormal}
                  />
                  {errors.username && <p id="af-username-error" role="alert" className="mt-1 text-xs text-red-600">{errors.username}</p>}
                </div>
                <div>
                  <label htmlFor="af-email" className="block text-sm font-medium text-gray-700 mb-1.5">
                  ...
                  />
                  {errors.email && <p id="af-email-error" role="alert" className="mt-1 text-xs text-red-600">{errors.email}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="af-role" className="block text-sm font-medium text-gray-700 mb-1.5">Role</label>
                  <select
                    id="af-role"
                    value={form.role}
                ...
                <div>
                  <label htmlFor="af-status" className="block text-sm font-medium text-gray-700 mb-1.5">Status</label>
                  <select
                    id="af-status"
                    value={form.status}
              ...
              <div className="pt-1 border-t border-gray-100">
                <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">
                  ...
                  {!isCreate && <span className="ml-1 font-normal normal-case tracking-normal text-gray-400">(leave blank to keep current)</span>}
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="af-password" className="block text-sm font-medium text-gray-700 mb-1.5">
                      Password {isCreate && <span className="text-red-500" aria-hidden="true">*</span>}
                    </label>
                    <input
                      id="af-password"
                      type="password"
                      ...
                      aria-describedby={errors.password ? 'af-password-error' : undefined}
                      className={errors.password ? inputError : inputNormal}
                    />
                    {errors.password && <p id="af-password-error" role="alert" className="mt-1 text-xs text-red-600">{errors.password}</p>}
                  </div>
                  <div>
                    <label htmlFor="af-confirm" className="block text-sm font-medium text-gray-700 mb-1.5">
                      Confirm {isCreate && <span className="text-red-500" aria-hidden="true">*</span>}
                    </label>
                    <input
                      id="af-confirm"
                      type="password"
                      ...
                      aria-describedby={errors.confirmPassword ? 'af-confirm-error' : undefined}
                      className={errors.confirmPassword ? inputError : inputNormal}
                    />
                    {errors.confirmPassword && <p id="af-confirm-error" role="alert" className="mt-1 text-xs text-red-600">{errors.confirmPassword}</p>}
                  </div>
                </div>
              </div>
            </div>
        ...
        >
          ...
          <div className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 id="delete-dialog-title" className="text-sm font-bold text-gray-900">Remove Account</h2>
                <p className="mt-1.5 text-sm text-gray-500">
                  Are you sure you want to remove{' '}
                  <span className="font-semibold text-gray-800">{account.name}</span> (@{account.username})?
...
function SummaryCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className={`rounded-xl border bg-white p-4 shadow-sm border-gray-100`}>
      ...
      <p className={`mt-0.5 text-xs font-semibold uppercase tracking-widest ${color}`}>{label}</p>
    </div>
  );
}
...
export function AccountManagementPage() {
  ...
  // ── CRUD handlers
  function handleCreate(data: Omit<AdminAccount, 'id' | 'createdAt' | 'lastLogin'>) {
  ...
  function handleEdit(data: Omit<AdminAccount, 'id' | 'createdAt' | 'lastLogin'>) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <SummaryCard label="Total Accounts" value={accounts.length} color="text-gray-400" />
        <SummaryCard label="Active" value={totalActive} color="text-green-600" />
        <SummaryCard label="Super Admins" value={totalAdmins} color="text-blue-600" />
        <SummaryCard label="Editors" value={totalEditors} color="text-violet-600" />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        ...
        {filtered.length === 0 && (
          ...
          >
            ...
            <p className="text-sm font-medium text-gray-400">No accounts found.</p>
            ...
            <table className="min-w-full divide-y divide-gray-100 text-sm">
              <thead>
                <tr className="bg-gray-50/60">
                  <th scope="col" className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Account</th>
                  <th scope="col" className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Role</th>
                  <th scope="col" className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</th>
                  <th scope="col" className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Joined</th>
                  <th scope="col" className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Last Login</th>
                  <th scope="col" className="px-5 py-3.5 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <motion.tbody
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-400 truncate">@{account.username} · {account.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4"><RoleBadge role={account.role} /></td>
                      <td className="px-5 py-4"><StatusBadge status={account.status} /></td>
                      <td className="px-5 py-4 text-sm text-gray-500">{formatDate(account.createdAt)}</td>
                      <td className="px-5 py-4 text-sm text-gray-500">
                        {account.lastLogin ? formatDateTime(account.lastLogin) : <span className="text-gray-300">Never</span>}
                      </td>
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            disabled={account.role === 'super-admin' && accounts.filter(a => a.role === 'super-admin').length === 1}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">@{account.username} · {account.email}</p>
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        <RoleBadge role={account.role} />
                        <span className="text-xs text-gray-400">Joined {formatDate(account.createdAt)}</span>
                      </div>
